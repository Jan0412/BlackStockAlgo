...
from  CovarianceFunctions import cov_SE
...
def compute_new_mu(k, k_s, y):
    k = np.asarray(k)
    k_s = np.asarray(k_s)

    y = np.asarray(y)

    return  k_s.dot(np.linalg.inv(k)).dot(y)
...
def compute_new_cov(k, k_s, k_ss):
    ...
    return k_ss - np.dot(k_s, np.dot(np.linalg.inv(k), k_s.T))
